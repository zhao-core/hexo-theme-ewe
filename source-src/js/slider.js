...
let localTagKey = 'ewe-tag'
let localSearchKey = 'ewe-search'
...
function init() {
	let app = new Q({
	    ...
	    filters: {
	    	...
	    	urlformat: (str) => {
				if (window.eweConfig && window.eweConfig.root) {
					return window.eweConfig.root + str
				}
	    		return '/' + str
	    	},
	    	tagformat: (str) => {
	...
	window.fetch(window.eweConfig.root + 'content.json?t=' + (+ new Date()), {
		method: 'get',
	}).then((res) => {
		return res.json()
	}).then((data) => {
	...
	let isTagOn = 'false'
	if (localTag === null) {
		isTagOn = (window.eweConfig && window.eweConfig.showTags) ? 'true' : 'false'
	} else {
